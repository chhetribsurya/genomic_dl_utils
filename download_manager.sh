...
log_message() {
    ...
    case "$level" in
        ...
        "SUCCESS") echo -e "${GREEN}${CHECKMARK} [${timestamp}]${NC} $message" ;;
        "ERROR") echo -e "${RED}${CROSSMARK} [${timestamp}]${NC} $message" ;;
        "WARN")  echo -e "${YELLOW}⚠ [${timestamp}]${NC} $message" ;;
        "SLURM") echo -e "${PURPLE}${CLUSTER} [${timestamp}]${NC} $message" ;;
    esac
}
...
check_dependencies() {
    ...
    if [[ ${#missing_deps[@]} -gt 0 ]]; then
        ...
        for dep in "${missing_deps[@]}"; do
            case "$dep" in
                wget|md5sum)
                    echo -e "  ${CYAN}# For Ubuntu/Debian:${NC} sudo apt-get install $dep"
                    echo -e "  ${CYAN}# For macOS:${NC} brew install $dep"
                    ;;
                sbatch|squeue|scancel)
                    echo -e "  ${CYAN}# SLURM tools - contact your system administrator${NC}"
                    ;;
            esac
        done
        return 1
...
download_urls() {
    ...
    if [[ "$use_slurm" == true ]]; then
        ...
        echo -e "${CYAN}${ARROW} Source file:${NC} $url_file"
        echo -e "${CYAN}${ARROW} Target directory:${NC} $target_dir"
        echo -e "${CYAN}${ARROW} Total URLs:${NC} ${BOLD}$total_urls${NC}"
        echo -e "${CYAN}${ARROW} Parallel jobs:${NC} ${BOLD}$max_jobs${NC}"
        echo -e "${CYAN}${ARROW} Timeout:${NC} ${timeout}s"
        echo -e "${CYAN}${ARROW} Retries:${NC} $retries"
        echo -e "${CYAN}${ARROW} Resume:${NC} $([ "$resume" == true ] && echo "enabled" || echo "disabled")"
        echo -e "${CYAN}${ARROW} Started:${NC} $(date '+%Y-%m-%d %H:%M:%S')"
        echo ""
        ...
        echo ""
        echo -e "${BOLD}${PURPLE}╔══════════════════════════════════════╗${NC}"
        echo -e "${BOLD}${PURPLE}║           Download Summary            ║${NC}"
        echo -e "${BOLD}${PURPLE}╚══════════════════════════════════════╝${NC}"
        ...
        echo -e "${GREEN}${CHECKMARK} Successful downloads:${NC} ${BOLD}$success_count${NC}"
        echo -e "${RED}${CROSSMARK} Failed downloads:${NC} ${BOLD}$error_count${NC}"
        echo -e "${CYAN}${ARROW} Total size:${NC} $(du -sh "$target_dir" 2>/dev/null | cut -f1 || echo 'Unknown')"
        echo -e "${CYAN}${ARROW} Completed:${NC} $(date '+%Y-%m-%d %H:%M:%S')"
        echo -e "${CYAN}${ARROW} Success log:${NC} $success_log"
        [[ -s "$error_log" ]] && echo -e "${CYAN}${ARROW} Error log:${NC} $error_log"
        echo ""
    fi
}
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            --help|-h)
                ...
                ;;
            --version|-v)
                echo -e "${BOLD}Advanced URL Downloader v${VERSION}${NC}"
